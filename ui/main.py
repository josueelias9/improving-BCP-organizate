...
from infrastructure.api_pdf_processing_repository import ApiPdfProcessingRepository
...
from use_cases.pdf_processing_use_cases import ProcessPdfUseCase
...
def init_dependencies():
    ...
    if 'repositories' not in st.session_state:
        st.session_state.repositories = {
            ...
            'pdf_processing': ApiPdfProcessingRepository()
        }
    ...
    if 'use_cases' not in st.session_state:
        ...
        st.session_state.use_cases = {
            ...
            'process_pdf': ProcessPdfUseCase(repos['file'], repos['pdf_processing'])
        }
...
def handle_file_upload(save_file_use_case, process_pdf_use_case):
    """Handle file upload"""
            ...
            if st.button("⚡ Procesar PDF", use_container_width=True):
                try:
                    with st.spinner("Procesando PDF..."):
                        result = process_pdf_use_case.execute()
...
def main():
    ...
    with st.sidebar:
        ...
        # File upload
        handle_file_upload(use_cases['save_file'], use_cases['process_pdf'])
