...
class ImportTransactionsFromCsvUseCase:
            ...
            for idx, row in enumerate(rows, start=2):  # Start at 2 because row 1 is header
                try:
                    ...
                    if result:
                        ...
                        skipped_count += 1
                        errors.append(f"Row {idx}: Transaction not updated: {row.get('unique_identifier', 'N/A')}")
    ...
    def _find_csv_file(self, filename: Optional[str] = None) -> Path:
        ...
        csv_files = list(output_dir.glob("transactions_*.csv"))
        
        if not csv_files:
            raise ValueError("No transaction CSV files found in output directory")
